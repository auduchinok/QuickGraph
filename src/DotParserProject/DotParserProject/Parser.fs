...
let parse str =
    ...
    | Error (pos, token, msg, debugFuns, _) ->
        failwithf "Error on position %d, token %A: %s" pos token msg
    | Success (ast, errors) ->
//        ast.EliminateCycles() // todo: try to remove
//        ast.ChooseSingleAst() // todo: try to remove
        translate translateArgs ast errors |> ignore
        data
